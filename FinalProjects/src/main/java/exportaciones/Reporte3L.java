 ...
 * @author oscar
 */
public class Reporte3L extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet Reporte3L</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet Reporte3L at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            response.setHeader(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=CuentaMayor.pdf");
            ...
            File file = new File(request.getServletContext().getRealPath("/resources/CuentaMayor.jrxml"));
            JasperReport jasperReports = JasperCompileManager.compileReport(file.getAbsolutePath());
            JRBeanCollectionDataSource dataSource = new JRBeanCollectionDataSource(lista);

            Map<String, Object> parameters = new HashMap<>();
            parameters.put("cuenta", cuenta.getCodigo());
            parameters.put("credito", cuenta.getCredito());
