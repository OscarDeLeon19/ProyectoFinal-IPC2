 ...
 * @author oscar
 */
public class Reporte6G extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet Reporte6G</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet Reporte6G at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            response.setHeader(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=Historial_Transacciones.pdf");

            String nombre = request.getParameter("nomCliente");
            ArrayList<Transaccion> lista = dmtra.historalTransacciones(nombre);

            File file = new File(request.getServletContext().getRealPath("/resources/HistorialTransacciones.jrxml"));
